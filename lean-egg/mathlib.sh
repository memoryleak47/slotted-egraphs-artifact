...
egg_expl_length_sum="$(cat "$log_dir/step-5-egg.txt" | grep -o '[0-9]*' | awk '{ sum += $1 } END { print sum }')"
slotted_expl_length_sum="$(cat "$log_dir/step-5-slotted.txt" | grep -o '[0-9]*' | awk '{ sum += $1 } END { print sum }')"
...
with_binders_egg_expl_length_sum="$(echo "$egg_with_binders_successes" | grep -o '[0-9]*' | awk '{ sum += $1 } END { print sum }')"
with_binders_slotted_expl_length_sum="$(echo "$slotted_with_binders_successes" | grep -o '[0-9]*' | awk '{ sum += $1 } END { print sum }')"
